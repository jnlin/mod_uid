...
module MODULE_VAR_EXPORT uid_module;
...
static void make_cookie(request_rec *r, cookie_dir_rec *dcfg)
{
	...
	ntohl(r->connection->local_addr.sin_addr.s_addr);
    ...
    ap_table_setn(r->notes, "uid_set", ap_pstrcat(r->pool,cname,"=",cbuf,NULL));
    
    // then recode
    for (i=0; i < sizeof(lk)/sizeof(lk[0]);i++)
	lk[i] = htonl(lk[i]);

    len = ap_base64encode_len(sizeof(lk));
    cookbuf = (char *) ap_palloc(r->pool, 1 + len);
    ap_base64encode_binary(cookbuf,(char*)lk,sizeof(lk));
    cookbuf[len] = '\0';
    when = r->request_time + cdelta;
    tms = gmtime(&when);
	...
	new_cookie 
	    = ap_psprintf(r->pool,
			  ...
			  cname, cookbuf, cpath,ap_day_snames[tms->tm_wday],
	...
	new_cookie 
	    = ap_psprintf(r->pool,
			  ...
			  cname, cookbuf,cpath, cdomain,ap_day_snames[tms->tm_wday],
			  tms->tm_mday, ap_month_snames[tms->tm_mon],
			  tms->tm_year % 100,
			  tms->tm_hour, tms->tm_min, tms->tm_sec);
    ...
    // put P3P header
    if (p3ph && !ap_table_get(r->headers_out,"P3P") && 
	((dcfg->p3p_enabled == ON && cdomain) ||dcfg->p3p_enabled == ALWAYS ))
	ap_table_setn(r->headers_out,"P3P",p3ph);
...
static int spot_cookie(request_rec *r)
{
    ...
    if ((cptr = (char*)ap_table_get(r->headers_in, "Cookie")))
	if (cname && cptr)
	    {
		char *cend;
		int clen;
		...
		if(cptr && cend)
			...
			while(*cend && *cend==' ')
			    cend++;
                        ...
                        cookiebuf = ap_pstrdup(r->pool, cend);
			cookieend = strchr(cookiebuf, ';');
			if (cookieend)
			    *cookieend = '\0';      /* Ignore anything after a ; */
			
			dlen = ap_base64decode_len(cookiebuf);
			dbuf = ap_palloc(r->pool,dlen+16);
			ap_base64decode_binary(dbuf,cookiebuf);
			...
			ap_table_setn(r->notes, "uid_got", 
				      ap_pstrcat(r->pool,cname,"=",cbuf,NULL));
...
static const char *set_service (cmd_parms *cmd, void *mconfig, char *name)
{
    cookie_dir_rec *dcfg = (cookie_dir_rec *) mconfig;
...
static const char *set_domain (cmd_parms *cmd, void *mconfig, char *name)
{
    cookie_dir_rec *dcfg = (cookie_dir_rec *) mconfig;
    dcfg->domain = ap_pstrdup(cmd->pool, name);
    return NULL;
}

static const char *set_path (cmd_parms *cmd, void *mconfig, char *name)
{
    cookie_dir_rec *dcfg = (cookie_dir_rec *) mconfig;
    dcfg->path = ap_pstrdup(cmd->pool, name);
    return NULL;
}

static const char *set_p3p (cmd_parms *cmd, void *mconfig, char *arg)
{
    cookie_dir_rec *dcfg = (cookie_dir_rec *) mconfig;
...
static const char *set_p3ps (cmd_parms *cmd, void *mconfig, char *arg)
{
    cookie_dir_rec *dcfg = (cookie_dir_rec *) mconfig;
    dcfg->p3p_header = ap_pstrdup(cmd->pool,arg);
    return NULL;
}
...
static void uid_init(server_rec *main_server, pool *p)
{
  sequencer=SEQ_MIN;
}

static const command_rec uid_cmds[] = {
    {"UIDExpires", set_cookie_exp, NULL, OR_FILEINFO, TAKE1,
     "UID cookie an expiry date code"},
    {"UIDActive", set_cookie_enable, NULL, OR_FILEINFO, FLAG,
     "whether or not to enable UID cookies"},
    {"UIDCookieName", set_cookie_name, NULL, OR_FILEINFO, TAKE1,
     "name of the tracking UID cookie"},
    {"UIDService",set_service,NULL,OR_FILEINFO,TAKE1,
     "UID Cookie service number"},
    {"UIDDomain",set_domain,NULL,OR_FILEINFO,TAKE1,
     "UID Cookie domain name"},
    {"UIDPath",set_path,NULL,OR_FILEINFO,TAKE1,
     "UID Cookie path"},
    {"UIDP3P",set_p3p,NULL,OR_FILEINFO,TAKE1,
     "UID P3P enable"},
    {"UIDP3PString",set_p3ps,NULL,OR_FILEINFO,RAW_ARGS,
     "UID P3P Header value"},
    {NULL}
};

static void * 
merge_uid_dir (ap_pool *p, void *basev, void *overridesv)
{
...
static void *make_uid_dir(pool *p, char *d)
{
    cookie_dir_rec *dcfg;
